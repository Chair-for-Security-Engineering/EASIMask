...
object ToSpinal extends SpinalMode
...
case class SpinalConfig(mode                           : SpinalMode = null,
                        ...
                        applyMasking                   : Boolean = false,
												maskingConfig 								 : MaskingConfig = MaskingConfig(),
  ...
  def generateSystemVerilog[T <: Component](gen: => T): SpinalReport[T] = Spinal(this.copy(mode = SystemVerilog))(gen)
	def generateSpinal [T <: Component](gen: => T): SpinalReport[T] = Spinal(this.copy(mode = ToSpinal))(gen)
  ...
  def apply[T <: Component](config: SpinalConfig)(gen: => T): SpinalReport[T] = {
    ...
    val report = configPatched.mode match {
      ...
      case `SystemVerilog` => SpinalVerilogBoot(configPatched)(gen)
			case `ToSpinal` => SpinalSpinalBoot(configPatched)(gen)
...
object SpinalSpinal {
	def apply[T <: Component](config: SpinalConfig)(gen: => T): SpinalReport[T] = Spinal(config.copy(mode = ToSpinal))(gen)
	def apply[T <: Component](gen: => T): SpinalReport[T] = SpinalConfig(mode = ToSpinal).generate(gen)
}
