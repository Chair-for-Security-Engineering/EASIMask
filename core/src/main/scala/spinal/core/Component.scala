...
abstract class Component extends NameableByComponent with ContextUser with ScalaLocated with PostInitCallback with Stackable with OwnableRef with SpinalTagReady with OverridedEqualsHashCode with ValCallbackRec {
  ...
  /** Defines if a component is linear */
  private[core] var linear: Boolean = false
  def isLinear = linear
  def setAsLinear = {
    linear = true
    this.definitionNameNoMerge = true
  }
	
	/** Defines if a component is non-linear */
	private[core] var nonLinear: Boolean = false
	def isNonLinear = nonLinear
	def setAsNonLinear = {
		nonLinear = true
		this.definitionNameNoMerge = true
	}
  
  private[core] var replace: Boolean = false
  private[core] var replacement: String = ""
  private[core] var randomness: Int = 0
  private[core] var randomnessName: String = ""
  def toBeReplaced = replace
  def replaceWith = replacement
  def setRandomness (rand: Int) = this.randomness = rand
  def setAsReplace (name: String, randomness: Int, randName: String) = {
    this.replace = true
    this.replacement = name
    this.randomness = randomness
    this.randomnessName = randName
  }
  
  /** Contains the copies of the linear component */
  val sharedComponents = ArrayBuffer[Component]()
...
//  def addPrePopTask(task: () => Unit) = Engine.get.onCompletion += task
  ...
  private[core] def allocateNames(globalScope: NamingScope): Unit = {
    ...
    localNamingScope.allocateName(anonymPrefix)
		...
		//for (child <- children) println(child.getName())/
		//Fuck this loop in particular
    for (child <- children) {
      OwnableRef.proposal(child, this)
      if (child.isUnnamed) {
				//println(child.definitionName)
