...
class Vec[T <: Data](val dataType: HardType[T], val vec: Vector[T]) extends MultiData with collection.IndexedSeq[T] {
  ...
  /**
    * Elementwise XOR of two Vec (for now, only with data type Bits)
    * @example {{{
    * val a, b, c = Vec(Bits(2 bits), 2)
    * c := a^b
    * }}}
    * @param right Right operand
    * @return The vector containing the elementwise XORs
    */
  def ^(right: Vec[Bits]): Vec[Bits] = {
    
    if (this.length != right.length)
      SpinalError(s"Lengths of vectors ${this.length} and ${right.length} do not match")
    
    val res = Vec(Bits(this.dataType.getBitsWidth bits), this.size)
    for (i <- this.indices) {
      res(i) := this(i).asBits ^ right(i).asBits
    }
    res
  }
  
  def $[a <: Data](right: Data): Data = {
    
    right match {
      case b: Bits => println("Bits"); //val res = Vec(Bits(this.dataType.getBitsWidth bits), this.size)
      case u: UInt => println("UInt"); //val res = Vec(Bits(this.dataType.getBitsWidth bits), this.size)
      case s: SInt => println("SInt"); //val res = Vec(Bits(this.dataType.getBitsWidth bits), this.size)
    }
    val res = Vec(UInt(this.dataType.getBitsWidth bits), this.size)
    /*T match {
      case b: Bits =>
    }*/
    /*right.dataType match {
      case b: Bits => Vec(Bits(2 bits), 2)
    }*/
    res
  }
  ...
  /**
    * Elementwise concatenation of two Vec (only for data type Bits)
    * @example {{{
    * val a, b = Vec(Bits(2 bits), 2)
    * val c = Vec(Bits(4 bits), 2)
    * c := a##b
    * /*c(1) := a(1)##b(1), c(0) := a(0)##b(0)*/
    * }}}
    * @param right Right operand
    * @return The Vector containing the elementwise concatenation of two input Vectors
    */
  def ##(right: Vec[Bits]): Vec[Bits] = {
    if (this.length != right.length)
      SpinalError(s"Lengths of vectors ${this} and ${right} do not match")
      
    val retWidth = 2*this.dataType.getBitsWidth
    val res = Vec(Bits(retWidth bits), this.size)
