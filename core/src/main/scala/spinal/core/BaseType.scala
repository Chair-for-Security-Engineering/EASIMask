...
abstract class BaseType extends Data with DeclarationStatement with StatementDoubleLinkedContainer[BaseType, AssignmentStatement] with Expression {
  ...
  /**
    * Clones and renames the BaseType. The new name will be "oldname_i"
    * @param i The digit to be appended to the old name
    * @return The renamed BaseType
    */
  def cloneAndRename (i: Int): BaseType = {
    val bt = cloneBaseType(s"${this.getName()}_${i}")
    bt.clockDomain = this.clockDomain
    bt.btFlags = this.btFlags
    bt.removeTag(implementDataAsShared)
    bt.parentScope = this.parentScope
    bt.dir = this.dir
    bt.parent = this.parent
    bt
  }
  
  def cloneAndRename (name: String): BaseType = {
    val bt = if (name == "") cloneBaseType(s"${this.getName()}") else cloneBaseType(name)
    bt.clockDomain = this.clockDomain
    bt.btFlags = this.btFlags
    bt.removeTag(implementDataAsShared)
    //bt.parentScope = this.parentScope
  ...
  def cloneAndRename (name: String, p: ScopeStatement): BaseType = {
    val bt = if (name == "") cloneBaseType(s"${this.getName()}") else cloneBaseType(name)
    bt.clockDomain = this.clockDomain
    bt.btFlags = this.btFlags
    bt.removeTag(implementDataAsShared)
  ...
  def cloneBaseType(name: String): BaseType = {
    this match {
      case b: Bits => Bits(this.getBitsWidth bits).setName(name)
      case u: UInt => UInt(this.getBitsWidth bits).setName(name)
      case s: SInt => SInt(this.getBitsWidth bits).setName(name)
      case b: Bool => Bool().setName(name)
    }
  }
