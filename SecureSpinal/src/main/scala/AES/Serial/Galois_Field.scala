...
class G256Inv2Shares5Stages extends Component {
	...
	val r = new ClockingArea(ClockDomain(io.clk, io.reset))	{
		...
		mul1.io.clk := io.clk
		mul1.io.reset := io.reset
		mul1.io.x := a
		mul1.io.y := b
		mul1.io.m := io.m(27 downto 20)
		d := mul1.io.z
		...
		inv.io.x(0) := c(0) ^ d(0)
		inv.io.x(1) := c(1) ^ d(1)
		inv.io.m := io.m(19 downto 8)
		f := inv.io.z
		
		mul2.io.clk := io.clk
		mul2.io.reset := io.reset
		mul2.io.x := b3
		mul2.io.y := f
...
class G16Mul2SharesDepMul extends Component {
	...
	val r = new ClockingArea(ClockDomain(io.clk, io.reset))	{
		val xTimesBlindedY, xTimesM, xTimesY = Vec(Bits(4 bits), 2)
		val blindedY, xTimesMBlinded, X, Y = Vec(Reg(Bits(4 bits)) init(0), 2)
		
		X := io.x
		Y := io.y
		
		blindedY(0) := io.y(1) ^ io.m(7 downto 4)
		blindedY(1) := io.y(0) ^ io.m(7 downto 4)
		
		xTimesMBlinded(0) := xTimesM(0) ^ io.m(3 downto 0)
		xTimesMBlinded(1) := xTimesM(1) ^ io.m(3 downto 0)
		
		io.z(0) := xTimesY(0) ^ xTimesBlindedY(0) ^ xTimesMBlinded(0)
		io.z(1) := xTimesY(1) ^ xTimesBlindedY(1) ^ xTimesMBlinded(1)
		
		for (i <- 0 until 2) {
			val mul = new G16Mul
			mul.io.a := X(i)
			mul.io.b := blindedY(i)
			xTimesBlindedY(i) := mul.io.x
		}
...
class G4Mul2SharesDepMul extends Component {
	...
	val r = new ClockingArea(ClockDomain(io.clk, io.reset)) {
		val xTimesBlindedY, xTimesM, xTimesY = Vec(Bits(2 bits), 2)
		val blindedY, xTimesMBlinded, X, Y = Vec(Reg(Bits(2 bits)) init(0), 2)
		
		X := io.x
		Y := io.y
		
		blindedY(0) := io.y(1) ^ io.m(3 downto 2)
		blindedY(1) := io.y(0) ^ io.m(3 downto 2)
		
		xTimesMBlinded(0) := xTimesM(0) ^ io.m(1 downto 0)
		xTimesMBlinded(1) := xTimesM(1) ^ io.m(1 downto 0)
