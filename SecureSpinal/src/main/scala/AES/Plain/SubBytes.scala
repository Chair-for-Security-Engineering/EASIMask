...
class SubBytes() extends Component {
	...
	def createValues = for (i <- sboxValues) yield U(i)
	
	val sbox = Mem(UInt(8 bits), 256) init(createValues)
	
	io.subBytesOutput := B(sbox(U(io.subBytesInput)))
}
...
class CanrightSBoxMasked extends Component {
	...
	/* Transform Input and Masks */
	val transform1, transform2, transform3, transform4, transform5 = new NewBasis
	...
	/* Transform a^(-1) back */
	transform4.io.input := AInv
	transform4.io.direction := True
	io.Q := transform4.io.output ^ B"8'h63"
}
