...
case class AES_2ndOrder_UnsharedKey() extends Component {
	val io = new Bundle {
		val clk = in  Bool()
		val reset = in  Bool()
	...
	val r = new ClockingArea(ClockDomain(io.clk, io.reset)) {
		...
		io.c(0) := mixColumnsOut(0) ^ io.k
		io.c(1) := mixColumnsOut(1)
		io.c(2) := mixColumnsOut(2)
	}
}

object AES_2ndOrder_UnsharedKey extends App {
	SpinalConfig(mode = VHDL, targetDirectory = "C:/Users/fabusbo/Desktop/VE-HEP/ap3/SpinalHDL/SecureSpinal/src/main/scala/AES/JRB/RTL").generate(AES_2ndOrder_UnsharedKey())
}
