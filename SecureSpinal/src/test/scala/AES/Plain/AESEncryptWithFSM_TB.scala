...
class AESEncryptWithFSM_TB extends AnyFunSuite {
	var compiled: SimCompiled[AESEncryptWithFSM] = null
	var i = 0
	
	test("compile") {
		compiled = SimConfig.withWave.compile(new AESEncryptWithFSM)
	}
	
	test("generateHDL") {
		SpinalConfig(mode = VHDL, targetDirectory = "SecureSpinal/src/main/scala/AES/Plain/RTL").generate(new AESEncryptWithFSM)
