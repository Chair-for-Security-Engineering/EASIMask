...
class AesEncrypt_Unrolled_TB extends AnyFunSuite {
	var compiled: SimCompiled[AesEncrypt_Unrolled] = null
	var i = 0
	
	test("compile") {
		compiled = SimConfig.withWave.compile(new AesEncrypt_Unrolled)
	}
	
	test("generateHDL") {
		SpinalConfig(mode = VHDL, targetDirectory = "SecureSpinal/src/main/scala/AES/Plain/RTL").generate(new AesEncrypt_Unrolled)
