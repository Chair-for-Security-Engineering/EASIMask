package Cryptol.KECCAK

import spinal.core._
import spinal.core.sim._
import org.scalatest.funsuite.AnyFunSuite

import scala.util.Random

class KECCAK_TB extends AnyFunSuite {
	var compiled: SimCompiled[KECCAK] = null
	
	test("Compile") {
		compiled = SimConfig.withWave.compile(new KECCAK)
	}
	
	test ("GenerateHDL") {
		SpinalConfig(mode = VHDL, targetDirectory = "/home/fabusbo/Schreibtisch/VE-HEP/ap3/SpinalHDL/SecureSpinal/src/main/scala/Cryptol/KECCAK/RTL").generate(new KECCAK)
	}
	
	test ("Testbench") {
		compiled.doSim(seed = 42) {dut =>
			
		 	dut.io.enable #= false
			ClockDomain(dut.io.clk, dut.io.reset).forkStimulus(10)
			sleep(160)
			dut.io.reset #= true
			sleep(100)
			
			dut.io.reset #= false
			dut.io.enable #= true
			//val p1 = BigInt("D300000000000000", 16)
			val p1 = BigInt("00000000000000D3", 16)
			val p2 = BigInt("0000000000000000", 16)
			val p3 = BigInt("0000000000000000", 16)
			val p4 = BigInt("0000000000000000", 16)
			val p5 = BigInt("0000000000000000", 16)
			val p6 = BigInt("0000000000000000", 16)
			val p7 = BigInt("0000000000000000", 16)
			val p8 = BigInt("0000000000000000", 16)
			val p9 = BigInt("0000000000000000", 16)
			val p10 = BigInt("0000000000000000", 16)
			val p11 = BigInt("0000000000000000", 16)
			val p12 = BigInt("0000000000000000", 16)
			val p13 = BigInt("0000000000000000", 16)
			val p14 = BigInt("0000000000000000", 16)
			val p15 = BigInt("0000000000000000", 16)
			val p16 = BigInt("0000000000000000", 16)
			val p17 = BigInt("0000000000000000", 16)
			//val p18 = BigInt("0000000000000080", 16)
			val p18 = BigInt("8000000000000000", 16)
			val p19 = BigInt("0000000000000000", 16)
			val p20 = BigInt("0000000000000000", 16)
			val p21 = BigInt("0000000000000000", 16)
			val p22 = BigInt("0000000000000000", 16)
			val p23 = BigInt("0000000000000000", 16)
			val p24 = BigInt("0000000000000000", 16)
			val p25 = BigInt("0000000000000000", 16)
			dut.io.keccakIn #= ((p1 << (24*64)) | (p2 << (23*64)) | (p3 << (22*64)) | (p4 << (21*64)) | (p5 << (20*64)) | (p6 << (19*64)) | (p7 << (18*64))
			 | (p8 << (17*64)) | (p9 << (16*64)) | (p10 << (15*64)) | (p11 << (14*64)) | (p12 << (13*64)) | (p13 << (12*64)) | (p14 << (11*64)) | (p15 << (10*64))
			 | (p16 << (9*64)) | (p17 << (8*64)) | (p18 << (7*64)) | (p19 << (6*64)) | (p20 << (5*64)) | (p21 << (4*64)) | (p22 << (3*64)) | (p23 << (2*64))
			 | (p24 << (1*64)) | (p25 << (0*64)))
			waitUntil(dut.io.done.toBoolean)
			sleep(10)
			assert(dut.io.keccakOut.toBigInt == BigInt("", 16))
			println(dut.io.keccakOut.toBigInt.toString(16).toUpperCase())
		}
	}
}

class KECCAKMasked_TB extends AnyFunSuite {
	var compiled: SimCompiled[KECCAKMasked] = null
	
	test("Compile") {
		compiled = SimConfig.withWave.compile(new KECCAKMasked)
	}
	
	test ("GenerateHDL") {
		SpinalConfig(mode = VHDL, targetDirectory = "/home/fabusbo/Schreibtisch/VE-HEP/ap3/SpinalHDL/SecureSpinal/src/main/scala/Cryptol/KECCAK/RTL").generate(new KECCAKMasked)
	}
	
	test ("Testbench") {
		compiled.doSim(seed = 42) {dut =>
			
			dut.io.enable #= false
			ClockDomain(dut.io.clk, dut.io.reset).forkStimulus(10)
			sleep(160)
			dut.io.reset #= true
			sleep(100)
			
			dut.io.reset #= false
			dut.io.enable #= true
			//val p1 = BigInt("D300000000000000", 16)
			val p1 = BigInt("00000000000000D3", 16)
			val p2 = BigInt("0000000000000000", 16)
			val p3 = BigInt("0000000000000000", 16)
			val p4 = BigInt("0000000000000000", 16)
			val p5 = BigInt("0000000000000000", 16)
			val p6 = BigInt("0000000000000000", 16)
			val p7 = BigInt("0000000000000000", 16)
			val p8 = BigInt("0000000000000000", 16)
			val p9 = BigInt("0000000000000000", 16)
			val p10 = BigInt("0000000000000000", 16)
			val p11 = BigInt("0000000000000000", 16)
			val p12 = BigInt("0000000000000000", 16)
			val p13 = BigInt("0000000000000000", 16)
			val p14 = BigInt("0000000000000000", 16)
			val p15 = BigInt("0000000000000000", 16)
			val p16 = BigInt("0000000000000000", 16)
			val p17 = BigInt("0000000000000000", 16)
			//val p18 = BigInt("0000000000000080", 16)
			val p18 = BigInt("8000000000000000", 16)
			val p19 = BigInt("0000000000000000", 16)
			val p20 = BigInt("0000000000000000", 16)
			val p21 = BigInt("0000000000000000", 16)
			val p22 = BigInt("0000000000000000", 16)
			val p23 = BigInt("0000000000000000", 16)
			val p24 = BigInt("0000000000000000", 16)
			val p25 = BigInt("0000000000000000", 16)
			val plain = ((p1 << (24*64)) | (p2 << (23*64)) | (p3 << (22*64)) | (p4 << (21*64)) | (p5 << (20*64)) | (p6 << (19*64)) | (p7 << (18*64))
			 | (p8 << (17*64)) | (p9 << (16*64)) | (p10 << (15*64)) | (p11 << (14*64)) | (p12 << (13*64)) | (p13 << (12*64)) | (p14 << (11*64)) | (p15 << (10*64))
			 | (p16 << (9*64)) | (p17 << (8*64)) | (p18 << (7*64)) | (p19 << (6*64)) | (p20 << (5*64)) | (p21 << (4*64)) | (p22 << (3*64)) | (p23 << (2*64))
			 | (p24 << (1*64)) | (p25 << (0*64)))
			val in1 = BigInt(1600, Random)
			val in2 = plain ^ in1
			dut.io.in1 #= in1
			dut.io.in2 #= in2
			waitUntil(dut.io.done.toBoolean)
			sleep(10)
			assert((dut.io.out1.toBigInt ^ dut.io.out2.toBigInt) == BigInt("", 16))
			//println(dut.io.keccakOut.toBigInt.toString(16).toUpperCase())
		}
	}
}

class Keccak_Wrapper_TB extends AnyFunSuite {
	var compiled: SimCompiled[KECCAK_Wrapper] = null
	
	test("Compile") {
		compiled = SimConfig.withWave.compile(new KECCAK_Wrapper)
	}
	
	test ("GenerateHDL") {
		SpinalConfig(mode = VHDL, targetDirectory = "/home/fabusbo/Schreibtisch/VE-HEP/ap3/SpinalHDL/SecureSpinal/src/main/scala/Cryptol/KECCAK/RTL").generate(new KECCAK_Wrapper)
	}
	
	test ("Testbench") {
		compiled.doSim(seed = 42) {dut =>
			
			dut.io.enable #= false
			ClockDomain(dut.io.clk, dut.io.reset).forkStimulus(10)
			sleep(160)
			dut.io.reset #= true
			sleep(100)
			
			dut.io.reset #= false
			dut.io.enable #= true
		
			dut.io.dataIn #= BigInt(800, Random)
			waitUntil(dut.io.done.toBoolean)
			sleep(10)
			//assert((dut.io.out1.toBigInt ^ dut.io.out2.toBigInt) == BigInt("", 16))
			//println(dut.io.keccakOut.toBigInt.toString(16).toUpperCase())
		}
	}
}